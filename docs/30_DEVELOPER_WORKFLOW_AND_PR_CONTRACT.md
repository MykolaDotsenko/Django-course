...
PR 3, PR 4/4.1 and PR 5 are now merged. The current-conversion product route is the primary web QA surface: initial server-rendered form, explicit first conversion, no-JavaScript POST, HTMX panel replacement/history, searchable picker, swap/focus restoration, stale trust metadata and browser race protection are all exercised through the same architecture boundaries used in production code.

The next bounded product slice is PR 6: historical conversion. It should extend the merged current-conversion flow with requested-date/effective-date semantics instead of creating a parallel form or client-side truth path.
